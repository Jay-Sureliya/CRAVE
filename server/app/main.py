...
# from sqlalchemy import or_  # <--- CRITICAL IMPORT FOR LOGIN FIX
# from passlib.context import CryptContext
# from datetime import datetime, timedelta, timezone
# from jose import jwt, JWTError
# import os
...
# @app.post("/login", response_model=TokenResponse)
# def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
#     # ✅ FIX: Check for Username OR Email
...
#     # 2. Check if restaurant is already active
#     if db.query(Restaurant).filter(Restaurant.email == request.email).first():
#         raise HTTPException(status_code=400, detail="Restaurant already active.")

#     # 3. Create ONLY the Request (Status = Pending)
#     new_request = RestaurantRequest(
#         restaurant_name=request.restaurantName,
#         owner_name=request.ownerName,
#         email=request.email,
#         phone=request.phone,
#         address=request.address,
#         status="pending"  # ✅ FIXED: Changed from 'approved' to 'pending'
#     )
#     db.add(new_request)
#     db.commit()
#     db.refresh(new_request)

#     return {"message": "Application Received! Waiting for Admin Approval.", "id": new_request.id}
