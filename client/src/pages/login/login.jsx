...
import { User, Lock, Mail, Phone, ArrowRight, Eye, EyeOff, Loader2, KeyRound, Hash, ArrowLeft, ChefHat, Star } from "lucide-react";
...
const AuthPage = () => {
    ...
    const googleLogin = useGoogleLogin({
        ...
        onError: (err) => {
            console.error("Google Auth Failed on Mobile:", err);
            // Added explicit error handling to let you know if HTTPS is required
            setError("Google Login Failed. If on mobile, ensure you are using a secure HTTPS connection.");
        },
        flow: 'implicit',
        ux_mode: 'redirect'
    });
                ...
                <div className="w-full max-w-[400px]">
                    ...
                    <AnimatePresence>
                        {forgotStage === 0 && (
                            ...
                            >
                                <motion.div
                                    // Left-1 fixes the white box positioning
                                    className="absolute h-[calc(100%-8px)] w-[calc(50%-4px)] top-1 left-1 bg-white rounded-lg shadow-sm"
                    ...
                    {forgotStage === 0 && (
                        <>
                            ...
                            <motion.button
                                layout
                                type="button" // FIXED: Prevents default form behaviours/reloads
                                disabled={isLoading} // FIXED: Disables multi-clicking
                                onClick={() => googleLogin()} // FIXED: Made as direct as possible to bypass mobile popup blockers
                                whileHover={{ backgroundColor: "#F9FAFB", scale: 1.01 }}
                                whileTap={{ scale: 0.99 }}
                                className="w-full bg-white border-2 border-gray-100 text-gray-700 font-bold text-sm py-3.5 rounded-xl flex items-center justify-center gap-3 transition-colors disabled:opacity-50"
                            >
                                <GoogleIcon />
                                Google
                            </motion.button>
