...
const Home = () => {
    return (
        <div className="font-sans text-slate-900 overflow-x-hidden bg-white flex flex-col items-center justify-center relative">
            ...
            {/* --- MAIN HERO CONTAINER --- */}
            <main className="relative w-[95%] bg-[#F9F9F9] rounded-[10px] overflow-hidden flex flex-col lg:flex-row min-h-[500px] sm:min-h-[600px] mt-4 lg:mt-8">

                {/* --- LEFT SIDE: TEXT CONTENT --- */}
                <div className="p-6 sm:p-8 lg:p-16 z-30 flex flex-col justify-center w-full lg:w-1/2">
                    <p className="text-slate-600 text-xs sm:text-sm font-semibold mb-3 sm:mb-4 tracking-wide text-center lg:text-left">
                        Order Restaurant food, takeaway and groceries.
                    </p>
                    ...
                    <div className="mt-2 w-full max-w-sm mx-auto lg:mx-0">
                        <p className="text-gray-500 text-[10px] sm:text-xs mb-2 sm:mb-3 ml-2 text-center lg:text-left">
                            Enter a postcode to see what we deliver
                        </p>
                        <div className="relative flex items-center w-full bg-white rounded-full border border-gray-200 shadow-lg p-1 sm:p-1.5 transition-all focus-within:ring-2 ring-orange-100">
                            <input
                                type="text"
                                placeholder="e.g. EC4R 3TE"
                                className="flex-grow bg-transparent px-4 sm:px-6 py-2 sm:py-3 text-sm sm:text-base text-slate-700 outline-none placeholder:text-slate-400 w-1/2"
                            />
                            <button className="bg-[#FF8A00] hover:bg-[#ff9f2e] text-white rounded-full px-5 sm:px-8 py-2.5 sm:py-3.5 font-bold text-xs sm:text-sm transition-colors shadow-md whitespace-nowrap">
                                Search
                            </button>
                        </div>
                    </div>
                ...
                {/* --- RIGHT SIDE: VISUALS --- */}
                <div className="flex-1 relative h-[400px] sm:h-[500px] lg:h-auto w-full overflow-hidden lg:overflow-visible">
                    ...
                    <div className="absolute top-0 right-0 h-full w-full z-0 flex justify-end">
                        <img
                            src={b3}
                            alt="Orange Background"
                            className="h-full object-cover sm:object-contain object-right-top opacity-90"
                        />
                    </div>

                    {/* 2. Pasta Girl Image */}
                    <div className="absolute top-8 sm:top-16 right-[10%] sm:right-[25%] lg:right-[35%] z-10 hidden md:block">
                        <div className="relative transform rotate-[-2deg] hover:rotate-0 transition-transform duration-500">
                            <img
                                src={b2}
                                alt="Eating Pasta"
                                className="w-[200px] sm:w-[260px] h-[260px] sm:h-[340px] object-cover rounded-[2rem] border-[6px] sm:border-[8px] border-white shadow-2xl"
                            />
                        </div>
                    </div>

                    {/* 3. Pizza Girl Image */}
                    <div className="absolute bottom-0 left-[-10%] sm:left-0 lg:left-[-280px] z-20 h-[75%] sm:h-[85%] w-[120%] sm:w-[130%] flex items-end justify-center lg:justify-start pointer-events-none">
                        <img
                            src={b1}
                            alt="Eating Pizza"
                            className="h-full object-contain mix-blend-multiply drop-shadow-xl"
                    ...
                    {/* 4. Floating Notification Cards (Static Design Elements) */}
                    <div className="absolute top-1/2 right-2 sm:right-4 lg:right-12 -translate-y-1/2 z-30 flex flex-col gap-3 sm:gap-5">
                        <NotificationCard
                            step="1"
                            title="We've Received your order!"
                            subtitle="Awaiting Restaurant acceptance"
...
// --- HELPER COMPONENT FOR THE NOTIFICATIONS (Responsive Visuals) ---
const NotificationCard = ({ step, title, subtitle }) => {
    return (
        <div className="relative pl-4 sm:pl-6 group cursor-default">
            {/* The Large Number Behind */}
            <span className="absolute -left-2 sm:-left-4 -top-3 sm:-top-5 text-[60px] sm:text-[80px] font-black text-white/40 z-0 select-none group-hover:text-white/60 transition-colors">
                {step}
            </span>

            {/* The Card Content */}
            <div className="relative z-10 bg-white/90 backdrop-blur-sm border border-white/60 shadow-lg rounded-xl sm:rounded-2xl p-3 sm:p-4 w-[200px] sm:w-[280px] transition-transform hover:-translate-y-1 duration-300">
                <div className="flex justify-between items-center mb-1">
                    <span className="text-[9px] sm:text-[10px] font-bold text-slate-800 uppercase tracking-wider">
                        Order <span className="text-[#FF8A00]">●</span>
                    </span>
                    <span className="text-[9px] sm:text-[10px] text-slate-400 italic">now</span>
                </div>
                <h3 className="text-[11px] sm:text-xs font-bold text-slate-900 leading-tight">
                    {title}
                </h3>
