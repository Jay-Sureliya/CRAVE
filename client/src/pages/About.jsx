...
const Counter = ({ end, duration = 2000 }) => {
    ...
    return (
        <span ref={ref} className="text-4xl md:text-5xl font-extrabold bg-gradient-to-br from-orange-600 to-amber-500 bg-clip-text text-transparent">
            {count.toLocaleString()}+
        </span>
    );
};
            ...
            {isOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1, y: 0 }}
                        exit={{ opacity: 0, scale: 0.9, y: 20 }}
                        className="bg-white rounded-3xl w-full max-w-lg shadow-2xl pointer-events-auto relative overflow-hidden flex flex-col z-10 max-h-[90vh] overflow-y-auto"
                    >
                        {/* --- DARK HEADER --- */}
                        <div className="bg-slate-900 p-6 pb-10 md:p-8 md:pb-12 relative overflow-hidden shrink-0">
                            <div className="absolute top-0 right-0 w-32 h-32 bg-orange-500 rounded-full blur-[60px] opacity-20 pointer-events-none"></div>
                            <div className="absolute bottom-0 left-0 w-24 h-24 bg-blue-500 rounded-full blur-[50px] opacity-10 pointer-events-none"></div>
                            ...
                            <button onClick={onClose} type="button" className="absolute top-4 right-4 md:top-5 md:right-5 z-50 p-2 bg-white/10 hover:bg-white/20 text-white rounded-full transition-all duration-200 cursor-pointer">
                                <X className="w-4 h-4 md:w-5 md:h-5 pointer-events-none" />
                            </button>
                        </div>

                        {/* --- CONTENT SECTION --- */}
                        <div className="px-6 md:px-8 pb-6 md:pb-8 relative flex-1 bg-white">
                            {/* Floating Badge */}
                            <div className="absolute -top-6 left-6 md:-top-8 md:left-8">
                                <div className={`h-12 w-12 md:h-16 md:w-16 rounded-2xl shadow-lg flex items-center justify-center rotate-3 border-4 border-white ${type === 'restaurant' ? 'bg-orange-600 shadow-orange-600/30' : 'bg-blue-600 shadow-blue-600/30'}`}>
                                    {type === 'restaurant' ? <Store className="w-6 h-6 md:w-8 md:h-8 text-white" /> : <Bike className="w-6 h-6 md:w-8 md:h-8 text-white" />}
                                </div>
                            </div>
                                ...
                                {!submitted ? (
                                    <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5">
                                        ...
                                        <div className="pt-2 md:pt-4">
                                            <button type="submit" disabled={loading} className="w-full group relative overflow-hidden bg-slate-900 text-white font-bold py-3 md:py-4 rounded-xl shadow-xl transition-all hover:shadow-2xl hover:-translate-y-1 disabled:opacity-70 disabled:hover:translate-y-0">
                                                <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-orange-600 to-amber-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                                    ...
                                    <div className="text-center py-6 md:py-10">
                                        <div className="inline-block p-4 rounded-full bg-green-50 border border-green-100 mb-4 md:mb-6">
                                            <CheckCircle2 className="w-10 h-10 md:w-12 md:h-12 text-green-600" />
                                        </div>
                                        <h3 className="text-xl md:text-2xl font-black text-slate-900 mb-2">We hear you!</h3>
                                        <p className="text-slate-500 mb-6 md:mb-8 max-w-xs mx-auto text-xs md:text-sm leading-relaxed">
                                            Application received. Credentials will be sent to <span className="text-slate-900 font-bold">{formData.email}</span> shortly.
                                        </p>
                                        <button onClick={onClose} className="w-full py-3 md:py-4 bg-slate-100 text-slate-900 font-bold rounded-xl hover:bg-slate-200 transition-all text-sm md:text-base">Close Window</button>
                                    </div>
                                )}
                            </div>
                        </div>
...
const About = () => {
    ...
    return (
        <div className="font-sans text-slate-900 overflow-x-hidden">
            ...
            <section className="relative flex items-center justify-center overflow-hidden">
                <div className="relative max-w-7xl mx-auto py-16 md:py-28 px-4 text-center z-10">
                    <FadeIn>
                        <span className="inline-block px-3 py-1.5 md:px-4 md:py-1.5 mb-4 md:mb-6 text-[10px] md:text-xs font-bold tracking-widest uppercase text-orange-600 bg-orange-100 rounded-full">
                            Redefining Delivery
                        </span>
                        <h1 className="text-5xl md:text-6xl lg:text-8xl font-black tracking-tight mb-6 md:mb-8">
                            About <span className="bg-gradient-to-r from-orange-600 via-orange-500 to-amber-400 bg-clip-text text-transparent">CRAVE</span>
                        </h1>
                        <p className="max-w-3xl mx-auto text-slate-500 text-lg md:text-xl lg:text-2xl leading-relaxed font-light px-2">
                            We aren't just a delivery app. We are the bridge between your
                            <span className="text-slate-900 font-medium"> cravings</span> and the city's
                            <span className="text-slate-900 font-medium"> finest kitchens</span>.
                        </p>
            ...
            <section>
                <div className="w-[95%] mx-auto py-8 md:py-10">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 text-center">
                        {[
                            ...
                            <FadeIn key={i} delay={i * 0.1}>
                                <Counter end={stat.end} />
                                <p className="text-slate-500 font-medium mt-2 md:mt-3 uppercase tracking-wider text-xs md:text-sm">{stat.label}</p>
                            </FadeIn>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
                        {[
                            ...
                            <motion.div
                                key={i}
                                whileHover={{ y: -10 }}
                                className="p-8 md:p-10 bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden group"
                            >
                                <div className="mb-5 md:mb-6 p-3 bg-orange-50 w-fit rounded-2xl group-hover:scale-110 transition-transform">
                                    {item.icon}
                                </div>
            ...
            <section className="py-16 md:py-24 relative overflow-hidden bg-white">
                <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'radial-gradient(#0f172a 1px, transparent 1px)', backgroundSize: '32px 32px' }}></div>
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] md:w-[800px] md:h-[800px] bg-orange-100/40 rounded-full blur-[80px] md:blur-[120px] pointer-events-none"></div>

                <div className="w-[95%] mx-auto relative z-10">
                    <FadeIn>
                        <div className="text-center max-w-3xl mx-auto mb-12 md:mb-20 px-2">
                            <div className="inline-flex items-center gap-2 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-orange-50 border border-orange-100 text-orange-600 font-bold text-[10px] md:text-xs uppercase tracking-widest mb-4 md:mb-6">
                                <Zap className="w-3 h-3 fill-orange-600" />
                                <span>Our DNA</span>
                            </div>
                            <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 tracking-tight mb-4 md:mb-6">
                                Built on <span className="bg-gradient-to-r from-orange-600 to-amber-500 bg-clip-text text-transparent">Principles</span>.
                            </h2>
                            <p className="text-slate-500 text-base md:text-lg lg:text-xl leading-relaxed">
                                We don't just deliver food; we deliver on our promises. These are the core values that drive our technology and our partnerships.
                            </p>
                        </div>
                    </FadeIn>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
                        {[
                            ...
                            <motion.div
                                key={i}
                                whileHover={{ y: -8 }}
                                className="group relative p-6 md:p-8 h-full bg-white rounded-[2rem] md:rounded-[2.5rem] border border-slate-100 hover:border-transparent shadow-xl shadow-slate-200/40 hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-500 overflow-hidden flex flex-col"
                            >
                                <div className="absolute inset-0 bg-gradient-to-br from-orange-500 to-amber-500 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ease-out"></div>
                                <div className="absolute -right-10 -top-10 w-32 h-32 md:w-40 md:h-40 bg-orange-50 rounded-full blur-2xl md:blur-3xl opacity-0 group-hover:opacity-20 transition-opacity duration-500"></div>

                                <div className="relative z-10 flex flex-col h-full">
                                    <div className="w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-900 mb-6 md:mb-8 group-hover:bg-white/20 group-hover:border-white/20 group-hover:text-white transition-all duration-500 shadow-sm">
                                        {value.icon}
                                    </div>
                                    <h3 className="text-xl md:text-2xl font-bold text-slate-900 mb-2 md:mb-3 group-hover:text-white transition-colors duration-500">
                                        {value.text}
                                    </h3>
                                    <p className="text-sm md:text-base text-slate-500 leading-relaxed group-hover:text-orange-50 transition-colors duration-500">
                                        {value.desc}
                                    </p>
                                </div>
                            </motion.div>
            ...
            <section className="w-[95%] mx-auto py-16 md:py-24">
                <div className="bg-slate-900 rounded-[2rem] md:rounded-[3rem] p-8 py-12 md:p-12 lg:p-30 text-center text-white shadow-2xl shadow-slate-900/20 relative overflow-hidden group">
                    <div className="absolute inset-0 opacity-[0.1]" style={{ backgroundImage: 'radial-gradient(#ffffff 1px, transparent 1px)', backgroundSize: '32px 32px' }}></div>
                    <div className="absolute top-0 right-0 w-[400px] h-[400px] md:w-[600px] md:h-[600px] bg-gradient-to-br from-orange-600 to-amber-500 rounded-full blur-[80px] md:blur-[100px] opacity-20 group-hover:opacity-30 transition-opacity duration-1000 -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>

                    <FadeIn>
                        <h2 className="text-3xl md:text-4xl lg:text-6xl font-black mb-4 md:mb-6 tracking-tight relative z-10 leading-tight">
                            Ready to join the <span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-amber-400">revolution?</span>
                        </h2>
                        <p className="text-slate-400 text-base md:text-lg lg:text-xl max-w-2xl mx-auto mb-8 md:mb-12 font-medium relative z-10 px-2">
                            Choose your path and start your CRAVE journey today.
                        </p>
                        
                        <div className="flex flex-col lg:flex-row gap-4 md:gap-6 justify-center items-center relative z-10">
                            ...
                            <motion.button
                                ...
                                whileHover={{ scale: 1.05, y: -5 }}
                                whileTap={{ scale: 0.95 }}
                                className="w-full md:w-auto flex items-center justify-center gap-3 md:gap-4 px-6 md:px-8 py-4 md:py-5 rounded-xl md:rounded-2xl bg-gradient-to-r from-orange-600 to-amber-600 text-white font-bold shadow-lg shadow-orange-900/20 hover:shadow-orange-500/40 transition-all"
                            >
                                <ShoppingBasket className="w-5 h-5 md:w-6 md:h-6 text-white" />
                                <div className="text-left">
                                    <span className="block text-[10px] md:text-xs opacity-80 uppercase tracking-widest text-orange-100">Hungry?</span>
                            ...
                            <motion.button
                                ...
                                whileHover={{ scale: 1.05, y: -5 }}
                                whileTap={{ scale: 0.95 }}
                                className="w-full md:w-auto flex items-center justify-center gap-3 md:gap-4 px-6 md:px-8 py-4 md:py-5 rounded-xl md:rounded-2xl bg-white text-slate-900 font-bold shadow-xl hover:bg-slate-50 transition-all"
                            >
                                <Store className="w-5 h-5 md:w-6 md:h-6 text-slate-900" />
                                <div className="text-left">
                                    <span className="block text-[10px] md:text-xs opacity-60 uppercase tracking-widest text-slate-500">Business</span>
                                    <span className="text-base md:text-lg">Add Restaurant</span>
                                </div>
                            </motion.button>
                            ...
                            <motion.button
                                ...
                                whileHover={{ scale: 1.05, y: -5 }}
                                whileTap={{ scale: 0.95 }}
                                className="w-full md:w-auto flex items-center justify-center gap-3 md:gap-4 px-6 md:px-8 py-4 md:py-5 rounded-xl md:rounded-2xl bg-slate-800 backdrop-blur-md text-white border border-slate-700 font-bold shadow-xl hover:bg-slate-700 transition-all"
                            >
                                <Bike className="w-5 h-5 md:w-6 md:h-6 text-slate-400 group-hover:text-white transition-colors" />
                                <div className="text-left">
                                    <span className="block text-[10px] md:text-xs opacity-60 uppercase tracking-widest text-slate-400">Earn Money</span>
                                    <span className="text-base md:text-lg">Become a Rider</span>
                                </div>
                            </motion.button>
                        </div>
                    </FadeIn>
