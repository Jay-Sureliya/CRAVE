...
import {
    User, LogOut, LayoutDashboard, UtensilsCrossed, ShoppingBag, ArrowLeft,
    ChevronRight, ShieldCheck, CreditCard, MapPin, X, Save, Lock, Mail,
    Home, Camera, UploadCloud, UserCircle
} from "lucide-react";

// --- IMPORT API SERVICE (Critical for consistent auth) ---
import api from "../../services/api";

// --- SUB-COMPONENTS ---
// Assuming these are in the components folder
...
const RestaurantDashboard = () => {
    ...
    // --- FETCH DATA (Updated to use 'api' instance) ---
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                // api.get automatically adds the Bearer token from interceptors
    ...
    const handleSaveProfile = async (e) => {
        ...
        try {
            ...
            setUserData(prev => ({
                ...prev,
                username: result.username || prev.username, // Fallback if backend doesn't return username
                name: result.name || prev.name, // Fallback if backend doesn't return name
